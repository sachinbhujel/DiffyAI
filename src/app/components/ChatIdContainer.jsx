...
function ChatIdContainer() {
        ...
        if (pathname === `/chat/${params.id}`) {
            get(params.id).then((modelchats) => {
                if (modelchats) {
                    ...
                    openaiGptOss120bChat.setMessages(
                        modelchats.chats.openaiGptOss120bChats
                    );
    ...
    // Get the title from localstorage and then update the title
    useEffect(() => {
...
export default ChatIdContainer;
